...
def main():
    ...
    parser.add_argument('--invoke', '-i', action='store_true', help='Run the function with the given workload')
    parser.add_argument('--fetch', '-f', action='store_true', help='Fetch log and config data from AWS')
    parser.add_argument('--train', '-t', action='store_true', help='Train the model based on the fetched log and config data')
    parser.add_argument('--recommend', '-r', action='store_true', help='Recommend a memory config based on the trained model')
    parser.add_argument('--profile', '-p', action='store_true', help='Test multiple memory configs to determine the optimal one')
    parser.add_argument('--model', '-m', type=str, default='linear', help='The ML model to use to train the model (default: linear)')
