...
class AWSPriceUnitRetriever(PriceUnitRetriever):
    ...
    def get_pricing_units(self) -> PricingUnits:
        ...
        for price in response["PriceList"]:
            ...
            price_dimensions = list(json.loads(price)["terms"]["OnDemand"].values())[0][
                "priceDimensions"
            ]
            ...
            if price_group == "AWS-Lambda-Duration-ARM":
                pricing["Execution time price ARM"] = max(
                    [
                        float(list(price_dimensions.values())[i]["pricePerUnit"]["USD"])
                        for i in range(len(price_dimensions))
                    ]
                )
        ...
        if architecture == "arm64":
            return PricingUnits(
                pricing["Execution time price ARM"], pricing["Request price ARM"]
            )
