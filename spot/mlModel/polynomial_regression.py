...
class PolynomialRegressionModel(MlModelBaseClass):
    def __init__(self, function_name, vendor, db: DBClient, last_log_timestamp, benchmark_dir):
        ...
        self._ml_model_file_path = os.path.join(self._benchmark_dir, "poly_regression_model.pkl")
        self._x = None
        self._y = None
        try:
    ...
    def train_model(self):
        ...
        self._plot_memory_size_vs_cost()
    ...
    def _plot_memory_size_vs_cost(self):
        ...
        plt.plot(xvars, np.polyval(self._model, xvars), label = self.get_polynomial_equation_string(), color = "red")
        ...
        plt.savefig(self._benchmark_dir+"/"+self._function_name + "-" + "polynomial_regression" + "-" + timestamp + ".png")
    ...
    def get_polynomial_equation_string(self):
        ...
        for degree in range(len(self._model)-1, 0, -1):
            if degree == 0:
                ...
                ret_val += str("{:.2E}".format(self._model[degree])) + "x^" + str(degree) + " + "  
        return ret_val
