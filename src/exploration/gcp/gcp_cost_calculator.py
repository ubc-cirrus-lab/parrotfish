...
class GCPCostCalculator(CostCalculator):
    ...
    def calculate_price(self, memory_mb: int, duration_ms: float or np.ndarray) -> float or np.ndarray:
        ...
        cpu_usage_mhz = (memory_mb / 128) * 200  # The CPU usage in Mhz, this is proportional to the memory config

        allocated_memory = 1.0 / 1024 * memory_mb  # convert MB to GB
        allocated_cpu = cpu_usage_mhz / 1000  # convert Mhz to Ghz
        request_compute_time = np.ceil(duration_ms/100) * 0.1  # convert ms to seconds 100 ms increments
        ...
        return self.pricing_units["request"] + memory_compute_charge + cpu_compute_charge
    ...
    def _get_pricing_units(self) -> dict:
        ...
        if product_name is not None:
            ...
            # Handle the response
            for product in [result for result in page_result if self.region in result.service_regions]:
        ...
        try:
            ...
            raise CostCalculationError("Parsing the prices retrieved from GCP Cloud Billing API failed.")
